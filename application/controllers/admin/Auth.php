            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($result) {
                    ...
                    if ($result['is_admin'] == 1) {
                        //get  admin and company information to store in session
                        $this->load->model("admin/company_model", "company_model");
                        $company_info = $this->company_model->get_company_detail_by_user_id($result["admin_id"]);
                        $notifications = $this->company_model->get_notifications($result["admin_id"]);
                        $admin_data = array(
                            ...
                            "name" => ucwords($result['firstname'] . " " . $result['lastname']),
                            'is_admin_login' => true,

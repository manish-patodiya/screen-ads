...
if (!function_exists('auth_check')) {
    function auth_check()
    {
        ...
        } else if ($ci->session->has_userdata('is_user_login')) {
...
if (!function_exists('user_auth_check')) {
    function user_auth_check()
    {
        ...
        if (!$ci->session->has_userdata('is_user_login')) {
            redirect('user/auth', 'refresh');
        } else if ($ci->session->has_userdata('is_admin_login')) {
            redirect('admin/auth/login', 'refresh');
        }
    }
}
...
//check login auth
if (!function_exists('login_check_auth')) {
    function login_check_auth()
    {
        // Get a reference to the controller object
        $ci = &get_instance();
        if ($ci->session->has_userdata('is_user_login')) {
            redirect('user/screen', 'refresh');
        } else if ($ci->session->has_userdata('is_admin_login')) {

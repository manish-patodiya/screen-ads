        ...
        <section class="content">
            <div class="card card-default">
                ...
                <div class="card-body">
                    ...
                    <?php echo form_open(base_url("admin/playlist/edit/$edit_data->id"), ['class' => "login-form", 'id' => 'frm-edit-playlist']); ?>
                    <div class='row'>
                        ...
                        <div class="form-group col-md-2">
                            <label class="control-label"><?=trans('playlist_type')?><span class="req">
                                    *</span></label>
                            <select name="playlist_type" class='form-control' data-live-search="true"
                                id="playlist_type">
                                <option value="">Select Playlist Type</option>
                                <?php foreach ($playlist_type_list as $p) {?>
                                <option value="<?=$p->id?>" <?=$plid == $p->id ? 'selected' : ''?>><?=$p->title?>
                                </option>
                                <?php }?>
                            </select>
                        </div>
                        <div class="form-group col-md-3">
                            ...
                            <select name="flash_content" class='form-control' data-live-search="true"
                                id="flash_content">
                                <option value="">Select Flash Content</option>
                                <?php foreach ($flash_content_list as $f) {?>
                                <option value="<?=$f['id']?>"
                                    <?=$f['id'] == $edit_data->flash_content_id ? 'selected' : ''?>><?=$f['title']?>
                                </option>
                                <?php }?>
                            </select>
                        </div>
                        <div class="form-group col-md-3">
                            <label class="col-md-12 control-label"><?=trans('title')?><span class="req">
                                    *</span></label>
                            <div class="col-md-12">
                                <input name='title' type="text" class='form-control' value='<?=$edit_data->title?>'
                    ...
                    <div class='form-group <?=$plid ? '' : 'hidden'?>' style="height:100vh">
                        <div class="col-md-12 row mx-0 h-100">
                            ...
                            <div class="col-md-12 screen">
                                <div class='w-25'>
                                    ...
                                    <select name="media_type[]" class="form-control selectpicker my-3"
                                        ...
                                        <?php $slct = isset($contents[0]) && $value['id'] == $contents[0]->media_type_id ? 'selected' : ''?>
                                        <option value='<?=$value['id']?>' <?=$slct?>>
                                            <?=$value['media_type']?></option>
                                        <?php }?>
                                    </select>
                                    <select name="media_content[]" class="form-control selectpicker"
                                        data-live-search="true" id="media1" onchange='showMedia(this,1)'>
                                        <?php if (isset($contents[0]) && in_array($contents[0]->media_type_id, [1, 2, 3, 8])) {?>
                                        <option value="">Select Media</option>
                                        <?php foreach ($contents[0]->medias as $k => $v) {?>
                                        <?php $slct = $v->id == $contents[0]->media_id ? 'selected' : ''?>
                                        <option value='<?=$v->id?>' name='<?=$v->media_file?>' <?=$slct?>>
                                            <?=$v->media_name?>
                                        </option>
                                        <?php }} else if (isset($contents[0]) && $contents[0]->media_type_id == 4) {?>
                                        <option value="">Select Template</option>
                                        <?php foreach ($temp_list as $k => $v) {?>
                                        <?php $slct = $v['id'] == $contents[0]->template_id ? 'selected' : ''?>
                                        <option value="<?=$v['id']?>" <?=$slct?>><?=$v["title"]?>
                                        </option>
                                        <?php }} else if (isset($contents[0]) && in_array($contents[0]->media_type_id, [5, 6, 7])) {?>
                                        <option value="">Select Group</option>
                                        <?php foreach ($contents[0]->media_groups as $k => $v) {?>
                                        <?php $slct = $v->id == $contents[0]->media_group_id ? 'selected' : ''?>
                                        <option value="<?=$v->id?>" <?=$slct?>><?=$v->group_name?>
                                        </option>
                                        <?php }} else if (isset($contents[0]) && $contents[0]->media_type_id == 9) {?>
                                        <option value="">Select Template</option>
                                        <?php foreach ($appointment_templates as $k => $v) {?>
                                        <?php $slct = $v['id'] == $contents[0]->user_appointment_tmplt ? 'selected' : ''?>
                                        <option value="<?=$v['id']?>" <?=$slct?>><?=$v['title']?>
                                        </option>
                                        <?php }}?>
                                    </select>
                            ...
                            <div class="col-md-6 screen">
                                <div class="w-50">
                                    ...
                                    <select name="media_type[]" class="form-control my-3 selectpicker"
                                        ...
                                        <option value="">Select Type</option>
                                        <?php foreach ($media_type_list as $k => $value) {?>
                                        <?php $slct = isset($contents[1]) && $value['id'] == $contents[1]->media_type_id ? 'selected' : ''?>
                                        <option value='<?=$value['id']?>' <?=$slct?>>
                                            <?=$value['media_type']?></option>
                                        <?php }?>
                                    </select>
                                    <select name="media_content[]" class="form-control selectpicker"
                                        ...
                                        <?php if (isset($contents[1]) && in_array($contents[1]->media_type_id, [1, 2, 3, 8])) {?>
                                        <option value="">Select Media</option>
                                        <?php foreach ($contents[1]->medias as $k => $v) {?>
                                        <?php $slct = $v->id == $contents[1]->media_id ? 'selected' : ''?>
                                        <option value='<?=$v->id?>' name='<?=$v->media_file?>' <?=$slct?>>
                                            <?=$v->media_name?>
                                        </option>
                                        <?php }} else if (isset($contents[1]) && $contents[1]->media_type_id == 4) {?>
                                        <option value="">Select Template</option>
                                        <?php foreach ($temp_list as $k => $v) {?>
                                        <?php $slct = $v['id'] == $contents[1]->template_id ? 'selected' : ''?>
                                        <option value="<?=$v['id']?>" <?=$slct?>><?=$v["title"]?>
                                        </option>
                                        <?php }} else if (isset($contents[1]) && in_array($contents[1]->media_type_id, [5, 6, 7])) {?>
                                        <option value="">Select Group</option>
                                        <?php foreach ($contents[1]->media_groups as $k => $v) {?>
                                        <?php $slct = $v->id == $contents[1]->media_group_id ? 'selected' : ''?>
                                        <option value="<?=$v->id?>" <?=$slct?>><?=$v->group_name?>
                                        </option>
                                        <?php }} else if (isset($contents[1]) && $contents[1]->media_type_id == 9) {?>
                                        <option value="">Select Template</option>
                                        <?php foreach ($appointment_templates as $k => $v) {?>
                                        <?php $slct = $v['id'] == $contents[1]->user_appointment_tmplt ? 'selected' : ''?>
                                        <option value="<?=$v['id']?>" <?=$slct?>><?=$v['title']?>
                                        </option>
                                        <?php }}?>
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class='h-50 screen row'>
                                    <div class="w-50">
                                        <input type="hidden" name='order[]' value='2'>
                                        <select name="media_type[]" class="form-control my-3 selectpicker"
                                            onchange="getcontent(this.value,2)" data-live-search="true"
                                            id="media-type2">
                                            <option value="">Select Type</option>
                                            <?php foreach ($media_type_list as $k => $value) {?>
                                            <?php $slct = isset($contents[1]) && $value['id'] == $contents[1]->media_type_id ? 'selected' : ''?>
                                ...
                                <div class='h-50 screen row'>
                                    <div class="w-50">
                                        ...
                                        <select name="media_type[]" class="form-control my-3 selectpicker"
                                            onchange="getcontent(this.value,3)" data-live-search="true"
                                            id="media-type3">
                                            <option value="">Select Type</option>
                                            <?php foreach ($media_type_list as $k => $value) {?>
                                            <?php $slct = isset($contents[2]) && $value['id'] == $contents[2]->media_type_id ? 'selected' : ''?>
                                            <option value='<?=$value['id']?>' <?=$slct?>>
                                                <?=$value['media_type']?></option>
                                            <?php }?>
                                        </select>
                                        <select name="media_content[]" class="form-control selectpicker"
                                            ...
                                            <?php if (isset($contents[2]) && in_array($contents[2]->media_type_id, [1, 2, 3, 8])) {?>
                                            <option value="">Select Media</option>
                                            <?php foreach ($contents[2]->medias as $k => $v) {?>
                                            <?php $slct = $v->id == $contents[2]->media_id ? 'selected' : ''?>
                                            <option value='<?=$v->id?>' name='<?=$v->media_file?>' <?=$slct?>>
                                                <?=$v->media_name?>
                                            </option>
                                            <?php }} else if (isset($contents[2]) && $contents[2]->media_type_id == 4) {?>
                                            <option value="">Select Template</option>
                                            <?php foreach ($temp_list as $k => $v) {?>
                                            <?php $slct = $v['id'] == $contents[2]->template_id ? 'selected' : ''?>
                                            <option value="<?=$v['id']?>" <?=$slct?>><?=$v["title"]?>
                                            </option>
                                            <?php }} else if (isset($contents[2]) && in_array($contents[2]->media_type_id, [5, 6, 7])) {?>
                                            <option value="">Select Group</option>
                                            <?php foreach ($contents[2]->media_groups as $k => $v) {?>
                                            <?php $slct = $v->id == $contents[2]->media_group_id ? 'selected' : ''?>
                                            <option value="<?=$v->id?>" <?=$slct?>><?=$v->group_name?>
                                            </option>
                                            <?php }} else if (isset($contents[2]) && $contents[2]->media_type_id == 9) {?>
                                            <option value="">Select Template</option>
                                            <?php foreach ($appointment_templates as $k => $v) {?>
                                            <?php $slct = $v['id'] == $contents[2]->user_appointment_tmplt ? 'selected' : ''?>
                                            <option value="<?=$v['id']?>" <?=$slct?>><?=$v['title']?>
                                            </option>
                                            <?php }}?>
                                        </select>
                            ...
                            <div class="col-md-12 row h-50">
                                <div class="screen col-md-6">
                                    <div class='w-50'>
                                        <input type="hidden" name='order[]' value='1'>
                                        <select name="media_type[]" class="form-control  my-3 selectpicker"
                                            onchange="getcontent(this.value , 1)" data-live-search="true"
                                            id="media-type1">
                                            <option value="">Select Type</option>
                                            <?php foreach ($media_type_list as $k => $value) {?>
                                            <?php $slct = isset($contents[0]) && $value['id'] == $contents[0]->media_type_id ? 'selected' : ''?>
                            ...
                            <div class="col-md-12 row h-50">
                                ...
                                <div class='screen col-md-6'>
                                    <div class="w-50">
                                        ...
                                        <select name="media_type[]" class="form-control my-3 selectpicker"
                                            onchange="getcontent(this.value,4)" data-live-search="true"
                                            id="media-type4">
                                            <option value="">Select Type</option>
                                            <?php foreach ($media_type_list as $k => $value) {?>
                                            <?php $slct = isset($contents[3]) && $value['id'] == $contents[3]->media_type_id ? 'selected' : ''?>
                                            <option value='<?=$value['id']?>' <?=$slct?>>
                                                <?=$value['media_type']?></option>
                                            <?php }?>
                                        </select>
                                        <select name="media_content[]" class="form-control selectpicker"
                                            ...
                                            <?php if (isset($contents[3]) && in_array($contents[3]->media_type_id, [1, 2, 3, 8])) {?>
                                            <option value="">Select Media</option>
                                            <?php foreach ($contents[3]->medias as $k => $v) {?>
                                            <?php $slct = $v->id == $contents[3]->media_id ? 'selected' : ''?>
                                            <option value='<?=$v->id?>' name='<?=$v->media_file?>' <?=$slct?>>
                                                <?=$v->media_name?>
                                            </option>
                                            <?php }} else if (isset($contents[3]) && $contents[3]->media_type_id == 4) {?>
                                            <option value="">Select Template</option>
                                            <?php foreach ($temp_list as $k => $v) {?>
                                            <?php $slct = $v['id'] == $contents[3]->template_id ? 'selected' : ''?>
                                            <option value="<?=$v['id']?>" <?=$slct?>><?=$v["title"]?>
                                            </option>
                                            <?php }} else if (isset($contents[3]) && in_array($contents[3]->media_type_id, [5, 6, 7])) {?>
                                            <option value="">Select Group</option>
                                            <?php foreach ($contents[3]->media_groups as $k => $v) {?>
                                            <?php $slct = $v->id == $contents[3]->media_group_id ? 'selected' : ''?>
                                            <option value="<?=$v->id?>" <?=$slct?>><?=$v->group_name?>
                                            </option>
                                            <?php }} else if (isset($contents[3]) && $contents[3]->media_type_id == 9) {?>
                                            <option value="">Select Template</option>
                                            <?php foreach ($appointment_templates as $k => $v) {?>
                                            <?php $slct = $v['id'] == $contents[3]->user_appointment_tmplt ? 'selected' : ''?>
                                            <option value="<?=$v['id']?>" <?=$slct?>><?=$v['title']?>
                                            </option>
                                            <?php }}?>
                                        </select>
...
$(function() {
    ...
    playlistContent.map((p) => {
        showMedia('#media' + p.screen_id, p.screen_id)
    })

...
class Example_model extends CI_Model
{
    ...
    {
        $this->db->where('id', $id);
        $file = $this->db->get('uploaded_files')->row_array()['name'];
    ...
    {
        $this->db->select('count(*) as users,admin_roles.admin_role_title');
        $this->db->join('admin_roles', 'admin_roles.admin_role_id = admin.admin_role_id');
        $this->db->group_by('admin.admin_role_id');
    ...
    {
        $query = $this->db->get_where('countries', array('status', 1));
        return $query->result_array();
    }
